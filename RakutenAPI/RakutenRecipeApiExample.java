        ...
        try (Scanner scanner = new Scanner(System.in)) {
            while (true) {
                ...
                if (children.isEmpty()) {
                    ...
                    // サブカテゴリがある場合の処理
                    int col = 3;
                    int width = 40; // 最大幅40（全角2,半角1で計算、括弧は全角扱い）
                    for (int i = 0; i < children.size(); i++) {
                        String name = children.get(i).name.replace('(', '（').replace(')', '）');
                        int visualLen = 0;
                        for (int c = 0; c < name.length(); c++) {
                            ...
                            if (block == Character.UnicodeBlock.CJK_UNIFIED_IDEOGRAPHS
                                ...
                                || block == Character.UnicodeBlock.BASIC_LATIN && ch == '～') {
                                visualLen += 2;
                            } else {
                                visualLen += 1;
                            }
                        }
                        int pad = width - visualLen;
                        String paddedName = name + " ".repeat(Math.max(0, pad));
                        System.out.printf("%2d: %s", i + 1, paddedName);
                        if ((i + 1) % col == 0 || i == children.size() - 1) {
                            System.out.println();
                        }
                    }
